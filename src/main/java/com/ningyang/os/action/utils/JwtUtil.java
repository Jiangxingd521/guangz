...
public class JwtUtil {
     ...
     * JWT 生成密钥使用的密码
     */
    private static final String JWT_RULE = "sy_node";
    ...
    public static String buildJWT(String sub) {
        return buildJWT(sub, null, UUID.randomUUID().toString(), null, null, 6000);
    }
    ...
    public static String parseJWTToString(String claimsJws) {
        boolean flag = checkJWT(claimsJws);
        SecretKey key = generateKey(JWT_ALG, JWT_RULE);
        String parseToken;
        if (flag == true) {
            parseToken = parseJWT(key, claimsJws).getBody().getSubject();
        } else {
            parseToken = "sorry data";
        }
        return parseToken;
    }
